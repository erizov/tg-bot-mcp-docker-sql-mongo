...
def main():
    ...
    execute_query(
        f"SELECT id, title, content, due_at, created_at FROM notes "
        f"WHERE created_at >= '{three_days_ago}' ORDER BY created_at DESC",
    ...
    execute_query(
        f"SELECT id, title, content, due_at FROM notes "
        f"WHERE due_at IS NOT NULL AND due_at BETWEEN '{now}' AND '{tomorrow}' "
    ...
    execute_query(
        "SELECT id, title, content, due_at FROM notes "
        "WHERE title LIKE '%важно%' OR title LIKE '%Важно%'",
